...
function animationTimer(duration = 1000, delay = 0) {
  ...
  useEffect(
    () => {
      ...
      // clean up. replaces componentWillUnmount
      return () => {
...
// main hook
function useAnimation({ easing = 'linear', duration = 500, delay = 0 }) {
...
export default () => {
  ...
  const animations = delays.map(delay =>
    useAnimation({ easing: 'elastic', duration: 600, delay })
  );
